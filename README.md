...
## 数据帧
帧长度 - 4 ~ 256 字节
每帧数据的格式为：固定的起始头 ( AK 的 ASCII 码 0x41 + 0x4B ) 、引导区 ( Len 本帧数据长度 + Sum 校验和，校验和是本帧所有其他数据字节和的补码 )、数据区 ( 每个数据包含键值数据对 Key + Val )
...
int main(void) {
  ...
  for(;;) {
    ...
    case EVENT_TICK:		// 滴答时钟， 20 次/秒
      ...
      dataframe[9] = 0;
      dataframe[11] = 0;
      dataframe[3] = - dataframe[5] - dataframe[7] - dataframe[9] - // 校验和
        dataframe[11] - ('A' + 'K' + sizeof dataframe + KEY_MODE +
        KEY_LED + KEY_RESERVED + KEY_ERROR);
